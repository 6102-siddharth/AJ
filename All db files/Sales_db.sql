...
select category, count(category)  from sales group by category having count(category) > 10;
...
select product_name , count(product_name) from sales group by product_name having count(product_name) > 5;        -- question 6 & 7 no output but its correct 
select product_name, count(product_name) from sales group by product_name;

-- 7-	What is the average sale amount for products with more than 2 sales?
select product_name, avg(sale_amount) from sales group by product_name having count(product_name) > 2;
...
select product_name , max(sale_amount) from sales group by product_name;

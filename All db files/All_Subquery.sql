...
select productname, count(quantity) from orders group by productname;
...
select * from orders where orderdate > '2024-03-02' and '2024-04-02';
...
select * from orders;
select count(paymentmethod) from orders;
select * from orders where paymentmethod = (select count(quantity) from orders having count(quantity) > 1);

-- 7. Find customers who ordered products priced above the average for their city.

select customername, productname, price,city from orders where price > (select avg(price) from orders where city= orders.city );

select avg(city) from orders;
select city, avg(price) from orders group by city;
...
select productname , count(productname) from orders group by productname  having count(productname) > 2;


-- 10. Find orders where the price is higher than any price of orders placed in 'Mumbai'.
select * from orders where price > (select price from orders where city= 'mumbai' limit 1);

...
#Sayalee List cars of type Gasoline with prices higher than the average price of cars with the fuel type = 'Gasoline'.
select * from cars where price > ( select avg(price) from cars where fueltype = 'Gasoline') and fueltype =  'Gasoline'; 
select * from cars where fueltype =  'Gasoline' and price > ( select avg(price) from cars where fueltype = 'Gasoline'); 

-- select avg(price) from cars;
-- (select brand, price,fueltype ,avg(price) from cars group by brand, price,fueltype having fueltype='gasoline') >  (select avg(price) from cars) ;
-- select brand, price,avg(price) from cars where fueltype='gasoline';
...
select brand , model, mileage from cars where mileage = (select max(mileage) from cars);
...
select brand,count(model) from cars group by brand having count(model) = (select count(model) from cars group by brand order by count(model) desc limit 1);


-- 5.	Retrieve cars with prices closest to the average price, rounded to the nearest hundred.
select avg(price) from cars;
select * from cars where round(price,-2) = (select avg(price) from cars);
...
select * from cars;
select enginetype, count(model) from cars group by enginetype having count(model) = (select count(model) from cars where enginetype= 'v8' );

-- select * from cars;
-- select enginetype,  count(enginetype) from cars group by enginetype;
-- select enginetype, count(model) from cars group by enginetype having  count(model) = (select count(model) from cars where enginetype= 'v8');
...
select brand, model , max(mileage) from cars 
group by brand , model
 having max(mileage) = (select max(mileage) from cars 
 group by brand, model 
 order by max(mileage) desc limit 1 );

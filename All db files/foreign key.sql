...
select o.orderid, o.customerid, o.orderdate ,o.quantity, o.totalamount, c.firstname, c.lastname 
from orders o join customers c on c.customerid = o.customerid;

-- 2. Display the product names and their respective categories.
select productname,category from products group by productname, category;

-- 3. List the order details, including product names, quantities, and prices.
select o.orderid,o.customerid, o.orderdate, o.quantity, p.productname,p.price,o.totalamount 
...
select c.customerid, c.firstname, c.lastname, o.orderdate from customers c join orders o on c.customerid= o.customerid ;

-- 5. Display the total number of orders placed by each customer.
select c.customerid, concat(c.firstname, " ", c.lastname ) as fullname,  count(o.orderid) 
...
select productid, Manufacturer from products ;


-- 7. Display the orders along with the product names and quantities ordered.
select o.orderid , p.productname, o.quantity from orders o join products p on o.productid = p.productid;


-- 8. List the customers who have not placed any orders.
select c.customerid, concat(c.firstname," ", c.lastname)as fullname,
 o.orderid from customers c left join orders o 
...
select c.customerid , concat(c.firstname, " ", c.lastname) as fullname , c.country 
from customers c  join orders o on c.customerid = o.customerid 
where c.country ='USA';

-- 11. Display the total amount spent by each customer on orders.
select c.customerid,concat(c.firstname, " ", c.lastname) as fullname,
 sum(totalamount) from customers c left join orders o 
...
select c.customerid,concat(c.firstname, " ", c.lastname) as fullname, 
count(orderid)  from customers  as c  left join orders as o on  c.customerid= o.customerid
 group by c.customerid, fullname;

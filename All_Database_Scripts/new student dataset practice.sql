...
select s.student_id,name,sum(p.amount) total_amount_paid 
from students as s join payments as p 
on s.student_id=p.student_id
group by student_id, name;

-- 8.	Find students who enrolled in 'Data' category 
select s.student_id, name, category 
from students s join enrollments e
on s.student_id=e.student_id
join courses c
on e.course_id=c.course_id
...
select s.student_id,name, payment_mode 
from students s join payments p
on s.student_id = p.student_id
...
-- 11.	Find top paying students Or TOp 5 paying students
select s.student_id, name ,sum(p.amount) as amounts
from students s join payments p
on s.student_id= p.student_id 
group by student_id, name
order by amounts desc limit 5;


-- 12.	Show most popular course 
select c.course_id, course_name, count(student_id) as Total_enrollments from courses c
join enrollments e
on c.course_id=e.course_id
...
select s.student_id, name,enrollment_id, count(enrollment_id) as counts
from students s left join enrollments e
on s.student_id= e.student_id
group by student_id, name, enrollment_id
having counts <=1;

-- 14.	Find courses with no enrollments 
select course_name , count(c.course_id) as course_counts
from courses as c join enrollments e 
on c.course_id= e.course_id
group by course_name 
having course_counts <=1;

-- 15.	Show students with multiple enrollments 
select name, count(e.student_id) as e_counts
from students s join enrollments e
on s.student_id = e.student_id
group by name
having e_counts >=1 ;
...
select s.student_id, name, e.course_id, course_name, category, p.payment_id, amount
from students s join enrollments e
on s.student_id= e.student_id
join payments p
on s.student_id=p.student_id
...
count(e.student_id) * price as Total_revenue from courses c
join enrollments e
on c.course_id=e.course_id
group by course_id, course_name
...
select category, count(e.student_id) as Total_enrollments ,
...
select name, max(amount) as Total_spent from students s
join payments p
on s.student_id= p.student_id
group by name
...
select c.course_id,course_name , count(e.student_id) as course_complition
from courses c join enrollments e
on c.course_id =e.course_id
where status = "completed"
group by course_id, course_name
order by course_complition desc;

-- 7. SUBQUERIES – 15 Questions
-- Basic
-- 1.	Find students who paid more than average payment 
-- select s.student_id, name, sum(p.amount) as Total_spend
-- from students s join payments p
-- on s.student_id= p.student_id
-- group by student_id, name 
-- having Total_spend > (select avg(amount) from payments);

-- 2.	Get courses with price greater than avg price 
select course_id, course_name from courses 
where price > (select avg(price) from courses);

					-- OR 
select course_id, course_name,price,(select avg(price) from courses ) as Avg_prices
from courses where price > (select avg(price) from courses);

-- 3.	Find students who enrolled in course_id = 101 
select distinct  s.student_id , name from students s
join enrollments e 
on s.student_id = e.student_id 
where course_id=101;

-- 4.	Find courses with max price 
select * from courses where price =(select max(price) from courses);

-- 5.	Get students with minimum payment 
select s.student_id , name, payment_id,amount from students s
join payments p
on s.student_id= p.student_id
where amount = (Select min(amount) from payments );

-- Intermediate
-- 6.	Find students who enrolled in more than 1 course 


-- 7.	Find courses with highest enrollments 

-- 8.	Get students who paid more than student_id 5 


-- 9.	Find second highest course price 


-- 10.	Find students with payment equal to max payment 


-- Advanced
-- 11.	Find students who never made payment 


-- 12.	Find courses not enrolled by any student 


-- 13.	Get students enrolled in all courses (tough 🔥) 


-- 14.	Find students with above average total payment 


-- 15.	Find course with lowest revenue



-- 8. WINDOW FUNCTIONS – 15 Questions (🔥 HIGHLY IMPORTANT)
-- 1.	Assign row number to students 


-- 2.	Rank courses by price 


-- 3.	Dense rank courses by duration 


-- 4.	Find top 3 highest paying students 


-- 5.	Find second highest payment 


-- 6.	Rank students by total payment 


-- 7.	Find cumulative revenue 


-- 8.	Find running total of payments 


-- 9.	Partition students by city and rank them 


-- 10.	Find top course per category 


-- 11.	Find lowest payment per student 


-- 12.	Find average payment per student (window) 


-- 13.	Find difference between current and previous payment 


-- 14.	Get first enrollment per student 


-- 15.	Find last payment per student

-- Create the database
CREATE DATABASE IF NOT EXISTS OnlineStoreDB1;
USE OnlineStoreDB1;
...
select Orderid, o.Customerid, Orderdate , 
...
select orderid,p.productname, o.quantity , p.price, Totalamount
 from orders as o join products as p
 on o.productid = p.productid;


-- 4. Retrieve a list of customers who made orders and the order dates.
select c.Customerid, Orderid, concat(c.firstname, " ", c.lastname) as Fullname, Orderdate 
...
select c.Customerid, concat(c.firstname," ",c.lastname) as Fullname,
 count(o.orderid) from customers as c left join orders as o 
on c.customerid=o.customerid 
...
select * from orders;
select o.Orderid , p.Productid,Quantity ,Productname
 from orders as o join products as p
on o.productid=p.productid;


-- 8. List the customers who have not placed any orders.
select c.customerid, concat(firstname," ",lastname) as fullname, o.orderid 
...
select orderid, o.customerid, orderdate, quantity, concat(c.firstname," ",c.lastname) as fullname , country from customers c join orders o
on c.customerid=o.customerid;

-- 11. Display the total amount spent by each customer on orders.
select c.customerid, concat(c.firstname," ",c.lastname) as fullname , Totalamount from customers c join orders as o
on c.customerid= o.customerid ;
...
select * from orders;
select orderid, orderdate, totalamount from orders where totalamount > 500;

-- 14. Show the customers along with the number of orders they've placed.
select c.customerid, concat(firstname, " ", lastname) as fullname ,orderdate ,count(c.customerid) from customers as c join orders as o
on c.customerid=o.customerid
group by c.customerid, fullname, orderdate ;
select * from customers;
select * from orders;

-- 15. Display products released after a specific date (e.g., '2022-01-01').
select * from products where releasedate > '2022-01-01' ;
select * from products;

...
select BusNumber, OperatorName, DestinationCity from BusDetails where DestinationCity = 'Goa' or DestinationCity='Nagpur' or DestinationCity ='Hyderabad';
...
select DepartureTime from BusDetails Where DepartureTime ='06:00:00';
...
select * from BusDetails order by DepartureTIme Desc limit 1;

-- 16.	Select BusNumber, SourceCity, DestinationCity where the destination city name ends with the letter e.
-- select BusNumber, SourceCity, DestinationCity from BusDetails where DestinationCity = '%e';
...
-- Optional--
select * from BusDetails  order by BusID asc limit 3;

-- 19.	Display BusID, BusType for buses whose BusType is Sleeper and arrival is after 06:00.
select BusId, BusType, ArrivalTime from BusDetails Where BusType ='Sleeper' And ArrivalTime > '06:00';
